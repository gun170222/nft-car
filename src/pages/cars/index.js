    ...
    <div className="cars-container">
      <div className="header pt-36 md:pt-52 lg:pt-60">
        <div className="relative py-40">
          <p className='top-0 z-50 text-5xl text-center text-white -translate-x-1/2 font-birosic md:text-6xl xl:text-9xl left-1/2'>cars</p>
          ...
          <p className="w-10/12 bottom-0 mx-auto mt-20 text-center text-white -translate-x-1/2 left-1/2">There’s a total of 12 models in gen-0. each having it’s own rarity level, wich Will determine it’s cometitive stats in game. Remi</p>
        </div>
        <div className="px-4 mx-auto lg:w-9/12">
          <p className="py-20 text-5xl text-white font-birosic lg:text-6xl">Trait</p>
            ...
            <div className="relative w-full sm:px-10">
              <Carousel
                ...
                {traitsList.map((car, idx) => (
                  carId === car.id && <CarItem car={car} key={idx} /> 
                ))}
              </Carousel>
            </div>
          </div>
        ...
        <div className="relative w-full sm:px-10">
          <Carousel
            ...
            {carsList.map((car, idx) => (
              <a href="#"><CarItem car={car} key={idx} carid={car.id} onClick={() => { setCarImage(car.img); setCarId(car.id); setCarName(car.name); setCarRarity(car.rarity); setCarAvailable(car.available); setCarAcceleration(car.acceleration); setCarWearAbility(car.wearability); setCarDescription(car.car_description);}}/></a>
            ))} 
          </Carousel>
        </div>
        {/* <div className="grid gap-8 pb-10 sm:grid-cols-2 xl:grid-cols-3">
